...
async def webhook(request: Request):
    ...
    # INICIO
    if estado == "INICIO":
        enviar_texto(numero, "¡Hola! 👋 ¿Buscas *casa* o *apartamento*? 🏠")
        usuario["estado"] = "TIPO"
    ...
    # IMAGENES
        ...
        if info.get("afirmacion") == "si":
            ...
            if inmueble.get("img_1"):
                enviar_imagen(numero, inmueble["img_1"], inmueble.get("descripcion", ""))
    ...
    # AGENDAR HORA
        ...
        enviar_texto(
            numero,
            f"✅ Visita agendada para el {usuario['fecha_seleccionada'].strftime('%d/%m')} a las {hora}.\nUn asesor la confirmará 😊"
        )
        usuario["estado"] = "INICIO"
